...
# -*- coding: utf-8 -*-
"""
ROS2 Node for Fused Drivable Area Segmentation for Winter Conditions.

This script implements a ROS2 node for the "Winter" mission profile. It uses
two separate YOLO segmentation models to identify drivable surfaces: the
standard track and snow-covered ground. The outputs of these models are
...
- **Bézier Curve Path Planning:** Generates a robust path for snowy terrain.
...
class YoloBevFusedDrivableAreaNode(Node):
    """
    A ROS2 node for fused drivable area detection in winter conditions.

    This node subscribes to a compressed image topic, performs a BEV
    transformation, and uses two separate YOLO models (for regular
    drivable area and snow) to segment the environment. The resulting
    ...
    Attributes:
        ...
        sand_model (YOLO): Model for detecting snow (re-using 'sand' variable name).
    ...
    def __init__(self):
        """
        Initialize the YoloBevFusedDrivableAreaNode for winter conditions.

        This sets up the node, declares and gets parameters for two YOLO
        models (driveable area and snow), loads all models and BEV parameters,
        and creates publishers, subscribers, and the thread pool executor.
        """
        super().__init__('yolo_bev_fused_drivable_area_node')
        self.get_logger().info("--- YOLO BEV fused drivable-area node (Bézier curve fusion architecture) ---")
    ...
    def _process_planning_data(self, compressed_data):
        ...
        two separate models (driveable, snow), fuses their masks, calculates
        the steering angle, and publishes the results.
    ...
    def calculate_steering_from_area(self, area_mask):
        ...
        Note: The implementation is identical to the other Bézier drive nodes.
