...
class HsvVisionNode(Node):
    """
        A ROS2 node for object detection and drivable area identification using HSV color filtering.

        This node subscribes to image streams from a RealSense camera (color and depth)
        and a standard USB camera. It uses RealSense data for supply box tracking
        and for creating a drivable area mask based on a predefined HSV color range.
        The USB camera stream is used for detecting various markers and traffic lights.

        Parameters
        ----------
        proc_width : int, default: 640
            The width to which input images are resized for processing.
        proc_height : int, default: 480
            The height to which input images are resized for processing.
        supply_model_path : str, default: './tracking.pt'
            Path to the YOLO model file for supply box detection.
        marker_model_path : str, default: './vision_enemy.pt'
            Path to the YOLO model file for marker detection.
        traffic_model_path : str, default: './traffic_light.pt'
            Path to the YOLO model file for traffic light detection.
        lower_hsv_bound : str, default: '[0, 0, 50]'
            The lower bound of the HSV color range for drivable area masking, as a string list.
        upper_hsv_bound : str, default: '[100, 40, 220]'
            The upper bound of the HSV color range for drivable area masking, as a string list.

        Attributes
        ----------
        bridge : CvBridge
            A bridge to convert between ROS Image messages and OpenCV images.
        device : str
            The computing device ('cuda' or 'cpu') used for PyTorch operations.
        supply_model : YOLO
            The YOLO model object for supply box detection.
        marker_model : YOLO
            The YOLO model object for marker detection.
        traffic_detection_model : YOLO
            The YOLO model object for traffic light detection.
        lower_hsv_bound : np.ndarray
            The lower HSV bound for color filtering, converted to a NumPy array.
        upper_hsv_bound : np.ndarray
            The upper HSV bound for color filtering, converted to a NumPy array.
        kernel : np.ndarray
            A morphological kernel used for noise reduction in the mask.
        scaled_camera_intrinsics : dict or None
            The camera intrinsic parameters scaled to the processing resolution.
        distance_pub : rclpy.publisher.Publisher
            Publishes the 3D coordinates of the detected supply box.
        realsense_viz_pub : rclpy.publisher.Publisher
            Publishes the visualized output from the RealSense camera as a compressed image.
        usb_cam_viz_pub : rclpy.publisher.Publisher
            Publishes the visualized output from the USB camera as a compressed image.
        mask_pub : rclpy.publisher.Publisher
            Publishes the drivable area mask image generated by HSV filtering.
        depth_for_path_pub : rclpy.publisher.Publisher
            Publishes the depth image corresponding to the mask for path planning.
        info_for_path_pub : rclpy.publisher.Publisher
            Publishes the camera info corresponding to the mask for path planning.
        ts : message_filters.ApproximateTimeSynchronizer
            Synchronizes timestamps for the RealSense color, depth, and camera info topics.
        usb_cam_sub : rclpy.subscription.Subscription
            Subscribes to the compressed image topic from the USB camera.
...
def main(args=None):
    ...
    node = HsvVisionNode()
    print(HsvVisionNode.__doc__)  # Print the docstring of the class for reference
    try:
        rclpy.spin(node)
