...
# AUTHOR: Geoffrey Hinton (Modified for Triple-Model Fusion with Attentional ROI)
# DESCRIPTION:
# [Hinton's Advanced Fusion & Robust Pure Pursuit Logic - Triple Model Extension with ROI]
...
# 9. [융합 아키텍처 확장] 3개의 ONNX 모델(Drivable Area, Sand, Stone) 추론 결과를 실시간으로 융합하여 통합 경로 생성
...
class YoloBevFusedDrivableAreaNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("--- YOLO BEV Fused Drivable Area Planning Node (Hinton's Ultimate Triple-Fusion Architecture with ROI) ---")
