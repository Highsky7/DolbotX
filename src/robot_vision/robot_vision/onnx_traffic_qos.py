#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# FILE: onnx_traffic_qos.py
# AUTHOR: Geoffrey Hinton (Optimized & Fixed Version with Reliability Filter)
# DESCRIPTION:
...
class YoloVisionNode(Node):
    def __init__(self):
        ...
        self.led_pub = self.create_publisher(String, '/led_control', self.qos_profile_reliable_default)
        self.traffic_pub = self.create_publisher(String, '/traffic_command', self.qos_profile_reliable_default)
    ...
    def initialize_image_sync(self):
        realsense_img_sub = message_filters.Subscriber(self, CompressedImage, '/camera/color/image_raw/compressed', qos_profile=self.qos_profile_sensor_data)
        depth_sub = message_filters.Subscriber(self, Image, "/camera/aligned_depth_to_color/image_raw", qos_profile=self.qos_profile_sensor_data)
    ...
    def run_supply_tracking(self, color_image_to_draw, resized_depth_image, yolo_input_image):
        ...
        # [Hinton's Reliability Fix] 연속성 및 거리 필터링 로직
