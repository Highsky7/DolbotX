...
class YoloBevDrivableAreaNode(Node):
    ...
    def calculate_steering_from_area(self, area_mask):
        ...
        if num_points < 50:
            ...
            # 포인트가 너무 적으면 새 경로를 계산하지 않고 기존 경로를 그대로 사용
            if self.tracked_center_path_coeff is not None:
                # 기존 경로를 그대로 반환하기 위해 아래 로직으로 전달
                pass
            else:
                # 추적 중인 경로조차 없으면 실패 반환
                return None, {}
            ...
            center_points_y = unique_y[valid_counts] + self.bev_h // 2  # 원본 이미지 y좌표로 복원
