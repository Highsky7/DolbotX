    ...
    def realsense_callback(self, compressed_image_msg, depth_msg):
        try:
            ...
            if cv_depth_raw is None:
                self.get_logger().warn("Failed to decompress depth image due to data corruption or format mismatch. Skipping this frame.")
                return
            
            cv_depth = cv2.resize(cv_depth_raw, (self.proc_width, self.proc_height), interpolation=cv2.INTER_NEAREST)
