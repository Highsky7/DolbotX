...
# -*- coding: utf-8 -*-
"""
ROS2 Node for Drivable Area Detection without BEV Transformation.

This script implements a simplified path planning node that operates directly
in the camera's image space, forgoing the Bird's-Eye-View (BEV) transformation.
It uses a YOLO segmentation model to identify the drivable area.

Instead of a complex path-following algorithm like Pure Pursuit, this node
employs a simple proportional controller. It calculates the centroid of the
detected drivable area within a predefined Region of Interest (ROI) and
computes a steering angle proportional to the horizontal error between this
centroid and the center of the image.

Key Features:
- **No BEV:** All processing is done on the raw camera image.
- **Proportional Steering Control:** Simple and efficient steering logic based on
  the centroid of the drivable area.
- **Region of Interest (ROI):** Processing is focused on the lower part of the
  image to improve stability and reduce noise.
- **Efficient, Multi-threaded Processing:** Offloads heavy computation to a
  worker thread to keep the ROS2 communication responsive.
"""
...
class YoloDrivableAreaNode(Node):
    """
    A ROS2 node for drivable area detection and steering control in camera space.

    This node subscribes to a compressed image topic, uses a YOLO model to
    segment the drivable area directly on the image, and calculates a steering
    angle using a simple proportional controller based on the area's centroid.
    """
    _MORPH_KSIZE = 7
    ...
    def __init__(self):
        """
        Initialize the YoloDrivableAreaNode.

        This sets up the node, loads the YOLO model, and creates the necessary
        publishers and subscribers. It uses a proportional gain for steering
        instead of lookahead-based algorithms.
        """
        super().__init__('yolo_drivable_area_node')
        self.get_logger().info("--- YOLO Drivable Area Planning Node (BEV-Removed, Camera-Space Control with ROI) ---")
    ...
    def _process_planning_data(self, compressed_data_buffer):
        ...
        This function decodes the image, runs YOLO inference, filters the mask,
        calculates the steering angle, and publishes results.
        ...
        try:
            ...
            results = self.model(image_for_model, conf=self.yolo_confidence, verbose=False)
            result = results[0]
    ...
    def filter_drivable_mask(self, mask):
        ...
        Args:
            mask (np.ndarray): The raw binary mask from the model.
    ...
    def calculate_steering_from_area(self, area_mask):
        """
        Calculate steering angle based on the centroid of the drivable area.

        This method defines an ROI on the image. It finds the largest contour
        (drivable area) within this ROI, calculates its center (centroid), and
        then computes a steering angle using a proportional controller based on
        the horizontal distance of the centroid from the image center.
        ...
        Returns:
            tuple: A tuple containing:
                - (float | None): The calculated steering angle, or None.
