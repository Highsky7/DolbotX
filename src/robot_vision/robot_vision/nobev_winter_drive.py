...
# -*- coding: utf-8 -*-
"""
ROS2 Node for Fused Drivable Area Detection for Winter Conditions without BEV.

This script is tailored for the "Winter" mission profile. It operates directly
in the camera's image space, avoiding a Bird's-Eye-View (BEV) transformation.
It uses two separate YOLO segmentation models to identify drivable surfaces:
the standard track and snow. The outputs of these models are fused into a
single unified drivable area mask.
...
- **Dual-Model Fusion:** Combines results from a standard and a snow model.
...
class YoloFusedDrivableAreaNode(Node):
    """
    A ROS2 node for fused drivable area detection in camera space (Winter).

    This node loads two YOLO models (drivable area, snow), fuses their
    segmentation results, and computes a steering command based on the
    centroid of the combined drivable area.
    """
    _MORPH_KSIZE = 7
    ...
    _ROI_TOP_Y_RATIO = 0.5    # Start ROI at 50% from the image top
    _ROI_BOTTOM_Y_RATIO = 1.0 # End ROI at the bottom of the image
    _ROI_WIDTH_RATIO = 1.0    # Use the full width of the image

    def __init__(self):
        """
        Initialize the YoloFusedDrivableAreaNode.

        This sets up the node, loads the two YOLO models (drivable area, snow),
    ...
    def _process_planning_data(self, compressed_data_buffer):
        ...
        This function decodes the image, runs two YOLO models, fuses their
    ...
    def calculate_steering_from_area(self, area_mask):
        ...
        This method is identical to the other non-BEV drive nodes.
